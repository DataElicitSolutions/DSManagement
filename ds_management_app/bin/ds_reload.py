...
from setup import compress_app_update_checkpoint,csv_file, create_machine_types_filter_file
...
temp_apps_download_list_dir = splunk_lib_util.make_splunkhome_path(['etc', 'apps', 'ds_management_app', 'var', 'run','temp_apps_download_list'])
temp_dc_info_csv = splunk_lib_util.make_splunkhome_path(['etc', 'apps', 'ds_management_app', 'var', 'run', 'dc_info_csv.csv'])
...
def process_row(row):
    try:
        if is_within_last_24_hours(int(row["_time"])):
            ...
            required_apps = get_apps_for_input(required_unique_keys, csv_file, row['os'])
...
def main():
    try:
        ...
        splunk.Intersplunk.outputResults([{"status":"success", "_raw": "Successfully reloaded deployment server","summary":RELOAD_SUMMARY }])
