...
for tgz_file in "$output_path"*.tgz; do
    ...
    if [[ ! ${apps_from_phonehome[$tgz_app_name]+_} ]]; then
        echo "Removing unused .tgz file and directory for app: $tgz_app_name"
        rm "$tgz_file"
...
if [[ "$changes_made" == true ]]; then
    echo "Changes were made to apps. Restarting Splunk..."
    "$SPLUNK_HOME/bin/splunk" restart
else
    echo "No changes made to apps. Skipping Splunk restart."
fi