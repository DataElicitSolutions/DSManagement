...
# Function to sleep random amount of time
sleep_random_time() {
    # Sleep for a random duration between 1 and 100 seconds
    sleep_duration=$((RANDOM % 10 + 1))
    echo "Sleeping for $sleep_duration seconds..." >> $SCRIPT_LOG_FILE
...
phonehome_url1="https://$dsIP:8089/servicesNS/-/ds_management_app/Phonehome"
phonehome_file_name=$guid"_"$ip"_"$host"_linux.txt"
phonehome_url2="http://$dsIP:8000/en-GB/static/app/ds_management_app/apps_download_list/$phonehome_file_name"
header="Content-Type: application/x-www-form-urlencoded"

apps_download_list="$SPLUNK_HOME/var/run/data/apps_download_list/"

# Check if the directory exists
if [[ ! -d "$apps_download_list" ]]; then
    echo "Directory $apps_download_list does not exist. Creating it..." >> $SCRIPT_LOG_FILE
    mkdir -p "$apps_download_list"
else
    echo "Directory $apps_download_list exists." >> $SCRIPT_LOG_FILE
fi

final_apps_download_list=$apps_download_list"/"$phonehome_file_name

success_download=true

response=$(curl -sk -w "%{http_code}" "$phonehome_url2" -o "$final_apps_download_list")

if [[ $response -ne 200 ]]; then
    echo "Failed to download $phonehome_file_name Retrying..." >> $SCRIPT_LOG_FILE
...
# Retry logic
if [ "$success_download" = false ]; then
    while [ $retry_count -lt $max_retries ]; do
    
        # Sleep for a random duration between 1 and 100 seconds
        sleep_random_time

        response=$(curl -sk -w "%{http_code}" "$phonehome_url1" --header "$header" \
        ...
        if [[ "$status_code" == "200" ]]; then
            response=$(curl -sk -w "%{http_code}"  "$phonehome_url2" -o "$final_apps_download_list")
            if [[ $response -ne 200 ]]; then
                echo "Failed to download $phonehome_file_name" >> $SCRIPT_LOG_FILE
                success=false
                break
            else
...
while IFS= read -r line; do
    ...
    response=$(curl -sk -w "%{http_code}" "$download_url" -o "$app_tgz_path")
